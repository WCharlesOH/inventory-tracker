...
if pagina == "🍔 Ventas":
    ...
    c1, c2, c3, c4, c5 = st.columns([1.1, 1.6, 1.2, 1.2, 1.2])
    with c1:
        st.text_input("DNI/RUC", key="dni_cliente", on_change=on_dni_enter)
...
# Historial
    ...
    with t2:
        ...
        if not compras:
            ...
            for comp in compras:
                with st.expander(f"#{comp.get('idOrden','?')} • S/ {comp.get('total',0):.2f}"):
                    c1, c2 = st.columns(2)
